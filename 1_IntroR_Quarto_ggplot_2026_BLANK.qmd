...
THIS TUTORIAL COMPLETED BY: (your name here)
...
```{r}
sweetness_plot <- ggplot(candy_rankings) + aes(x = chocolate, y = sugarpercent) + geom_boxplot()
sweetness_plot
```
...
```{r}
sweethist <- ggplot(candy_rankings) + aes(
  x = chocolate, y = winpercent) + geom_histogram()
sweethist
```
